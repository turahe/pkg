...
Role in architecture:
  ...
  - Single source of truth for server, database, Redis, storage, rate limiter, CORS, timezone, OpenTelemetry, Sentry, and mTLS settings.

Responsibilities:
  - Load and parse environment variables into typed structs (Configuration and nested types).
  - Server includes JWT settings: algorithm (default RS256), key paths or inline/embedded PEM, issuer, audience, key ID.
